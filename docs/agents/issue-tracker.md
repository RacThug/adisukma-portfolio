...
Issues and PRDs for this repo live as markdown files in `.scratch/`. There is no external
issue tracker, so pull requests are not a triage surface: everything enters through a file
under `.scratch/`.

`.scratch/` is gitignored. This is a public repo, and the planning trail is deliberately
kept local rather than published.
...
- Triage state is recorded as a `Status:` line near the top of each issue file. The role
  strings are in `triage-labels.md`
...
There are no labels in a file-based tracker. Set the `Status:` line at the top of the
issue file to the corresponding role string from `triage-labels.md`.
...
- **Child ticket**: `.scratch/<effort>/issues/NN-<slug>.md`, numbered from `01`, with the
  ...
  `grilling` / `task`); a `Status:` line records `claimed` / `resolved`.
